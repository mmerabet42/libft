...
/*   Updated: 2018/04/16 21:59:37 by mmerabet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		checkwild1(const char **str, t_mchi *cur, t_mchi *nxt, int i)
{
...
static int		checkwild2(const char **str, t_mchi *cur, t_mchi *nxt, int i)
{
...
static int		checktype(const char **str, t_mchi *cur, t_mchi *nxt)
{
	...
	if (cur->type == 1)
	{
		if ((!nxt && cur->len == -1) || nxt->whatever)
			return (1);
		if ((ret = checkwild1(str, cur, nxt, 0)) == 1)
			return (2);
		else if (ret == -1)
			return (0);
	}
	else if (cur->type == 2)
	{
		if (!checkmchi((*str), cur))
			return (0);
		if ((ret = checkwild2(str, cur, nxt, 0)) == 1)
			return (2);
		else if (ret == -1)
			return (0);
	}
...
int				ft_strtks(const char *str, t_mchi *mchi)
{
	...
	while (mchi && *str)
	{
		if (mchi->type == 0)
		{
			...
			if (!ft_strnequ(str, mchi->str, ft_strlen(mchi->str)))
				return (0);
			str += len;
			g_iread += len;
		}
